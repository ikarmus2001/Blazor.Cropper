...
/// Cropper component
/// </summary>
public partial class Cropper : ComponentBase, IAsyncDisposable
{
    [Inject]
    ...
    internal bool WiderThanContainer => _image.Width / _image.Height > _imgContainerWidth / _imgContainerHeight;

    internal double ImgRealW =>
        WiderThanContainer
            ? _imgContainerWidth * _imgSize / 100
            : _imgContainerHeight * _imgSize / 100 * _image.Width / _image.Height;

    internal double ImgRealH =>
        WiderThanContainer
            ? _imgContainerWidth * _imgSize / 100 * _image.Height / _image.Width
            : _imgContainerHeight * _imgSize / 100;
    ...
    internal string GetCropperStyle(double top, double left, double height, double width) => FormattableString.Invariant($"top:{top}px;left:{left}px;height:{height}px;width:{width}px;");

    internal string GetCroppedImgStyle(double top, double right, double bottom, double left) => FormattableString.Invariant($"clip: rect({top}px, {right}px, {bottom}px, {left}px);");
    ...
    internal (double resizeProp, double cw, double ch) GetCropperInfos()
    {
        ...
        resizeProp = WiderThanContainer ? _image.Width / _imgContainerWidth : _image.Height / _imgContainerHeight;
    ...
    internal TouchEventArgs MouseToTouch(MouseEventArgs args) => new()
    {
        Touches = new[]
        {
            new TouchPoint
